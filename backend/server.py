...
# Smoke Detector endpoints
@api_router.post("/smoke-detectors", response_model=SmokeDetector)
async def create_smoke_detector(detector: SmokeDetectorCreate):
...
@api_router.put("/smoke-detectors/{detector_id}", response_model=SmokeDetector)
async def update_smoke_detector(detector_id: str, update_data: SmokeDetectorUpdate):
    detector = await db.smoke_detectors.find_one({"id": detector_id})
    if not detector:
        raise HTTPException(status_code=404, detail="Smoke detector not found")
...
@api_router.delete("/smoke-detectors/{detector_id}")
async def delete_smoke_detector(detector_id: str):
...
# Fire Extinguisher endpoints
@api_router.post("/fire-extinguishers", response_model=FireExtinguisher)
async def create_fire_extinguisher(extinguisher: FireExtinguisherCreate):
...
@api_router.put("/fire-extinguishers/{extinguisher_id}", response_model=FireExtinguisher)
async def update_fire_extinguisher(extinguisher_id: str, update_data: FireExtinguisherUpdate):
    extinguisher = await db.fire_extinguishers.find_one({"id": extinguisher_id})
    if not extinguisher:
        raise HTTPException(status_code=404, detail="Fire extinguisher not found")
...
@api_router.delete("/fire-extinguishers/{extinguisher_id}")
async def delete_fire_extinguisher(extinguisher_id: str):
